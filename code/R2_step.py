# r2_classic_style_metrics.py
# -----------------------------------------------------------------
# 同样地，将古典学术风格应用到统计指标图上
#
# 格式变更:
# - 字体: 严格设为 10pt Times New Roman。
# - 线宽: 坐标轴边框、误差线、柱状图边框线宽统一设为 1.0pt。
# - 刻度线: 主刻度线朝向图内 (direction='in')。
# - 边框: 显示完整的四边黑色边框 (spines)。
# - 移除背景网格线。
# -----------------------------------------------------------------
...
SCENARIOS = ["6 dB", "10 dB", "15 dB", "Dropped"]
N_REP = 12

LOCK_MEAN_PID = [70, 96, 116, 116]
LOCK_MEAN_2DOF = [40, 55, 66, 80]
MAXDEV_MEAN_PID = [2.8, 3.8, 4.6, 4.0]
MAXDEV_MEAN_2DOF = [0.6, 0.8, 1.0, 1.0]
LOCK_SD_PID = [8, 10, 12, 12]
LOCK_SD_2DOF = [5, 6, 7, 8]
MAXDEV_SD_PID = [0.35, 0.45, 0.50, 0.50]
MAXDEV_SD_2DOF = [0.12, 0.16, 0.20, 0.20]
...
def plot_single_metric(ax, metric: MetricSet, ylab, title, ylim=None):
    ...
    for i in range(S):
        ...
        ax.scatter(np.full(N_REP, x[i] - width / 2) + jitter(N_REP, 0.08), metric.pid[i], s=2, alpha=0.7, color='orchid', zorder=10)
        ax.scatter(np.full(N_REP, x[i] + width / 2) + jitter(N_REP, 0.08), metric.d2f[i], s=2, alpha=0.7, color='dimgrey', zorder=10)
    ...
    # 3. 设置刻度线朝内
    ax.tick_params(axis='both', which='major', direction='in')# r2_classic_style_metrics.py
# -----------------------------------------------------------------
# 同样地，将古典学术风格应用到统计指标图上
#
# 格式变更:
...
LOCK_MEAN_PID   = [51, 70, 85, 92]     # 6 dB, 10 dB, 15 dB, Dropped
LOCK_MEAN_2DOF  = [12, 17, 20, 24]

LOCK_SD_PID     = [6,  8, 10, 11]
LOCK_SD_2DOF    = [2,  2,  3,  3]

# —— Max Deviation (%) ——（保持你前面给的结果，15 dB 已对齐：PID=5.0%、2-DOF=1.2%）
MAXDEV_MEAN_PID   = [2.8, 3.8, 5.0, 4.0]
MAXDEV_MEAN_2DOF  = [0.6, 0.8, 1.2, 1.0]

MAXDEV_SD_PID     = [0.30, 0.40, 0.55, 0.50]
MAXDEV_SD_2DOF    = [0.10, 0.12, 0.20, 0.18]
...
def plot_single_metric(ax, metric: MetricSet, ylab, title, ylim=None):
    ...
    # 调整柱状图边框线宽为 1.0
    ax.bar(x - width / 2, m1, width, label="PID", color=colors[0], alpha=0.8, edgecolor='black', linewidth=1.0)
    ax.bar(x + width / 2, m2, width, label="2-DOF", color=colors[1], alpha=0.8, edgecolor='black', linewidth=1.0)
    ...
    for i in range(S):
        ...
        ax.scatter(np.full(N_REP, x[i] - width / 2) + jitter(N_REP, 0.08), metric.pid[i], s=2, alpha=0.7, color='#dbc1a6', zorder=10)
        ax.scatter(np.full(N_REP, x[i] + width / 2) + jitter(N_REP, 0.08), metric.d2f[i], s=2, alpha=0.7, color='dimgrey', zorder=10)
...
def main():
    ...
    # --- 绘图 (调用已修改的函数) ---
    plot_single_metric(axes[0], LOCK, "lock time (ms)", "(a) Lock Time", ylim=(0, 150))
    plot_single_metric(axes[1], MAXD, "Max Deviation (%)", "(b) Max Deviation", ylim=(0, 10))
